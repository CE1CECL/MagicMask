...
import com.topjohnwu.magicmask.view.MagicMaskDialog.DialogClickListener

typealias DialogButtonClickListener = (DialogInterface) -> Unit

class MagicMaskDialog(
    context: Activity, theme: Int = 0
) : AppCompatDialog(context, theme) {

    private val binding: DialogMagicMaskBaseBinding =
        DialogMagicMaskBaseBinding.inflate(LayoutInflater.from(context))
    ...
    inner class ButtonViewModel : Button, ObservableHost {
        ...
        fun clicked() {
            onClickAction(this@MagicMaskDialog)
            if (!doNotDismiss) {
                dismiss()
            }
        }
