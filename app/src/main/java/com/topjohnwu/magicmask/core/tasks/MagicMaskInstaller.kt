package com.topjohnwu.magicmask.core.tasks
...
import com.topjohnwu.magicmask.StubApk
import com.topjohnwu.magicmask.core.*
import com.topjohnwu.magicmask.core.di.ServiceLocator
import com.topjohnwu.magicmask.core.utils.MediaStoreUtils
import com.topjohnwu.magicmask.core.utils.MediaStoreUtils.inputStream
import com.topjohnwu.magicmask.core.utils.MediaStoreUtils.outputStream
import com.topjohnwu.magicmask.core.utils.RootUtils
import com.topjohnwu.magicmask.ktx.reboot
import com.topjohnwu.magicmask.ktx.withStreams
import com.topjohnwu.magicmask.ktx.writeTo
import com.topjohnwu.magicmask.signing.SignBoot
...
abstract class MagicMaskInstallImpl protected constructor(
    ...
    private fun extractFiles(): Boolean {
        ...
        try {
            ...
            if (isRunningAsStub) {
                ...
                // Also extract magicmask32 on non 64-bit only 64-bit devices
                val is32lib = Const.CPU_ABI_32?.let {
                    { entry: ZipEntry -> entry.name == "lib/$it/libmagicmask32.so" }
                } ?: { false }
                ...
                // Also symlink magicmask32 on non 64-bit only 64-bit devices
                val lib32 = info.javaClass.getDeclaredField("secondaryNativeLibraryDir").get(info) as String?
                if (lib32 != null) {
                    libs += File(lib32, "libmagicmask32.so")
                }
    ...
    private fun processTar(input: InputStream, output: OutputStream): OutputStream {
        ...
        if (Config.recovery && recovery.exists() && boot.exists()) {
            ...
            arrayOf(
                "cd $installDir",
                "chmod -R 755 .",
                "./magicmaskboot unpack boot.img",
                "./magicmaskboot repack boot.img",
    ...
    private fun handleFile(uri: Uri): Boolean {
        ...
        try {
            uri.inputStream().buffered().use { src ->
                ...
                val filename = StringBuilder("magicmask_patched-${BuildConfig.VERSION_CODE}_").run {
                    for (i in 1..5) {
    ...
    private fun patchBoot(): Boolean {
        ...
        val job = shell.newJob().add("./magicmaskboot cleanup", "cd /")
...
abstract class MagicMaskInstaller(
    console: MutableList<String>,
    logs: MutableList<String>
) : MagicMaskInstallImpl(console, logs) {
    ...
    class Patch(
        ...
        console: MutableList<String>,
        logs: MutableList<String>
    ) : MagicMaskInstaller(console, logs) {
    ...
    class FixEnv(private val callback: () -> Unit) : MagicMaskInstallImpl() {
        override suspend fun operations() = fixEnv()
