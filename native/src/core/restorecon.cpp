...
#define MAGICMASK_CON  "u:object_r:" SEPOL_FILE_TYPE ":s0"
...
static void restore_magicmaskcon(int dirfd) {
    struct dirent *entry;

    fsetfilecon(dirfd, MAGICMASK_CON);
    fchown(dirfd, 0, 0);
    ...
    while ((entry = xreaddir(dir.get()))) {
        ...
        if (entry->d_type == DT_DIR) {
            restore_magicmaskcon(fd);
            continue;
        } else if (entry->d_type) {
            fsetfilecon(fd, MAGICMASK_CON);
            fchown(fd, 0, 0);
        }
        close(fd);
    }
...
void restore_databincon() {
    restore_magicmaskcon(xopen(DATABIN, O_RDONLY | O_CLOEXEC));
}

void restore_tmpcon() {
    if (!selinux_enabled())
        return;
    if (MAGICMASKTMP == "/sbin")
        setfilecon(MAGICMASKTMP.data(), ROOT_CON);
    else
        chmod(MAGICMASKTMP.data(), 0700);

    auto dir = xopen_dir(MAGICMASKTMP.data());
    ...
    if (SDK_INT >= 26) {
        string magicmask = MAGICMASKTMP + "/magicmask";
        setfilecon(magicmask.data(), EXEC_CON);
    }
}
