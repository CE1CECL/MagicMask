...
[[noreturn]] static void usage() {
    fprintf(stderr,
R"EOF(MagicMask - Multi-purpose Utility
...
int magicmask_main(int argc, char *argv[]) {
    if (argc < 2)
        usage();
    if (argv[1] == "-c"sv) {
#if MAGICMASK_DEBUG
        printf(MAGICMASK_VERSION ":MAGICMASK:D (" str(MAGICMASK_VER_CODE) ")\n");
#else
        printf(MAGICMASK_VERSION ":MAGICMASK:R (" str(MAGICMASK_VER_CODE) ")\n");
#endif
        return 0;
    } else if (argv[1] == "-v"sv) {
